...
| 更新块文本 | `edit_feishu_doc` | `document_id`, `operation=update_text`, `block_id`, `content` |
...
3. 调用对应写入 tool。
4. 返回结构化结果中的 `document_id`、`folder_token`、`url`、`task_id` 或写入块数量。
5. 只有用户明确需要同步检索索引时，才传 `index_after_create=true` 或 `refresh_index=true`。
