...
description: 使用飞书知识检索与原子读 MCP tools 查询项目文档、按关键词检索知识库、读取单篇文档摘要、查看最近更新、比较 revision 差异、列飞书文件夹、读取 docx blocks 或获取文件元信息。当用户提到飞书文档、项目文档、需求文档、最近更新、版本对比、文件夹清单、block_id、文件元数据时使用。
---
...
- 处理文档检索、摘要、最近更新、差异比较，以及明确 token/URL 下的飞书原子读取。
- 不创建文档，不修改文档，不维护任务状态，不承诺主动通知。
- 无法定位到唯一文档时，先返回候选，再继续读取摘要或比较差异。
- 原子读 tool 可以直接访问真实飞书；调用前必须有明确 `folder_token`、`document_id`、`token` 或 URL。
...
| 已知飞书文件夹 token，想看文件清单 | `list_folder_files` | 可设置 `recursive`、`max_depth`、`limit` |
| 已知 docx document_id，想定位 block_id | `get_docx_blocks` | 返回 blocks、plain_text、elements、parent_id |
| 已知文件 token 或 URL，想看标题/类型/链接 | `get_file_meta` | 无法从 token 推断类型时传 `file_type` |
...
6. 后续需要写入时，把原子读结果交给 `doc-write` 流程，先 dry-run 再确认写入。
...
- “列一下这个飞书文件夹里的文件”
  - 已知 `folder_token` 时调用 `list_folder_files`
- “我要更新这篇文档某一段，先帮我找 block_id”
  - 已知 `document_id` 时调用 `get_docx_blocks`
